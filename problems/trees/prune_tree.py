...
def prune_tree(tree: Tree, keys_to_discard):
    """
...
def do_test_prune_tree(trees_original_expected, tree_name, keys_to_prune):
    ...
    recreate_msg = tree_utils.gen_recreate_msg_with_trees(
        "prune_tree", tree_name, keys_to_prune
    )
    ...
    tree_utils.check_tree_unmodified(
        trees[tree_name], original_trees[tree_name], recreate_msg
    )
...
def test_prune_tree_10(trees_prune_tree):
    do_test_prune_tree(trees_prune_tree, "tree_10", {"V11", "V8", "V7", "V4", "V5"})
...
def test_prune_tree_12(trees_prune_tree):
    do_test_prune_tree(trees_prune_tree, "tree_12", {"V26", "V9", "V1", "V6", "V27"})
...
def test_prune_tree_14(trees_prune_tree):
    do_test_prune_tree(
        trees_prune_tree, "tree_14", {"V2", "V0", "V1", "V7", "V4", "V6", "V3", "V5"}
    )


def test_prune_tree_15(trees_prune_tree):
    do_test_prune_tree(
        trees_prune_tree, "tree_15", {"V2", "V48", "V49", "V46", "V45", "V47"}
    )
