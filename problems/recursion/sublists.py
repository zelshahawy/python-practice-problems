...
def sublists(lst: list[Any]) -> list[list[Any]]:
    """
    ...
    def backtracking(sublist: list[Any], index: int):
        if index == len(lst):
            result.append(sublist.copy())
            return

        backtracking(sublist, index + 1)
        backtracking(sublist + [lst[index]], index + 1)
...
def do_test_sublists(lst):
    ...
    expected = [
        [x for j, x in enumerate(lst) if i ^ (2**j) < i] for i in range(2 ** len(lst))
    ]
    actual = sublists(lst)
